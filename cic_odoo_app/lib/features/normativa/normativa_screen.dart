  ...
  Future<void> _load() async {
    ...
    try {
      final rows = await _odoo.searchRead(
        'calidad.normativa',
        fields: const ['name', 'codigo', 'fecha_publicacion', 'unidad_id'],
      ...
      try {
        final rows = await _odoo.searchRead('calidad.normativa', fields: const ['name'], order: 'id desc', limit: 120);
        _items = rows.map((e) => Map<String, dynamic>.from(e as Map)).toList();
      } catch (e) {
        _error = e.toString();
      }
    ...
    return Scaffold(
      ...
      body: _loading
          ...
          : _error != null
              ...
              : _items.isEmpty
                  ? const Center(child: Text('Sin normativa disponible.', style: TextStyle(color: AppTheme.textMuted)))
                  : ListView.separated(
                        ...
                        return Container(
                          ...
                          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            if ((it['codigo'] ?? '').toString().isNotEmpty)
                              Text(it['codigo'].toString(), style: const TextStyle(fontSize: 11, color: AppTheme.textMuted)),
                            Text((it['name'] ?? '').toString(), style: const TextStyle(fontWeight: FontWeight.w700, color: AppTheme.textPrimary)),
                            if ((it['fecha_publicacion'] ?? '').toString().isNotEmpty)
                              Text('Publicación: ${it['fecha_publicacion']}', style: const TextStyle(fontSize: 12, color: AppTheme.textSecondary)),
